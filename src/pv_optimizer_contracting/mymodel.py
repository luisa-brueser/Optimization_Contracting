...
# Sets
model.time = Set(initialize = set_time.keys(),doc="Set of time")
...
model.capacity_factor = Param(model.time,model.options,initialize =dict_capacity_factor,doc="Maximum available electricity supply per unit of installed capacity, per option at each timeperiod")
...
model.capacity=Var(model.options, within=NonNegativeReals,doc='Total installed capacity of one option, maximum available power. per option at each timeperiod')
#model.area_pv=Var()
# model.only_contractor_supplies=create_contractor_only_disjunct()
# model.only_grid_supplies=create_grid_only_disjunct()
# model.only_pv_supplies=create_pv_only_disjunct()
...
def cost_rule(model):
    return sum(model.supply[time, option] * model.price_elec[option] + model.capacity[option]*5
    for time in model.time 
    for option in model.options)

# def cost_rule(model):
#     return sum(model.supply[i]*model.price_elec[i]+model.y[i]*model.annuity*model.price_invest[i] for i in model.options)
...
# model.c6 = Constraint(model.time,model.options,rule=demand_rule, doc='Supply equals demand within the sum of all time steps')
...
# def production_rule(model,time,option):
#     return model.supply[time,option] <= model.capacity_factor[time,option]*model.capacity[option]

# model.c7 = Constraint(model.time,model.options,rule=demand_rule, doc='Supply is smaller or equal to max. production')


def production_rule(model,time,option):
    return model.supply[time,option] <= model.capacity[option]
